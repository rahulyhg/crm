...
use \Core\ORM\Entity;
use \Core\Entities\Team;

use \Core\Core\Exceptions\Error;
use \Core\Core\Exceptions\Forbidden;

class InboundEmail extends \Core\Services\Record
{
    ...
    public function getFolders($params)
    {
        ...
        $storage = new \Core\Core\Mail\Mail\Storage\Imap($imapParams);
    ...
    public function fetchFromMailServer(Entity $emailAccount)
    {
        ...
        $importer = new \Core\Core\Mail\Importer($this->getEntityManager(), $this->getConfig());
        ...
        $parserClassName = '\\Core\\Core\\Mail\\Parsers\\' . $parserName;
        ...
        foreach ($monitoredFoldersArr as $folder) {
            ...
            foreach ($ids as $i => $id) {
                ...
                $email = null;
                try {
                    $message = new \Core\Core\Mail\MessageWrapper($storage, $id, $parser);
    ...
    {
        $className = '\\Core\\Custom\\Business\\Distribution\\CaseObj\\RoundRobin';
        if (!class_exists($className)) {
            $className = '\\Core\\Modules\\Crm\\Business\\Distribution\\CaseObj\\RoundRobin';
        }
    ...
    {
        $className = '\\Core\\Custom\\Business\\Distribution\\CaseObj\\LeastBusy';
        if (!class_exists($className)) {
            $className = '\\Core\\Modules\\Crm\\Business\\Distribution\\CaseObj\\LeastBusy';
        }
    ...
    protected function emailToCase(\Core\Entities\Email $email, array $params = array())
    {
        ...
        try {
            ...
            if ($replyEmailTemplateId) {
                ...
                if (empty($contact)) {
                    ...
                    $fromName = \Core\Services\Email::parseFromName($email->get('fromString'));
                    if (!empty($fromName)) {
