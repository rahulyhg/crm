...
class AclManager
{
    ...
    protected $tableClassName = '\\Core\\Core\\Acl\\Table';
    ...
    public function getImplementation($scope)
    {
        if (empty($this->implementationHashMap[$scope])) {
            ...
            $className = '\\Core\\Custom\\Acl\\' . $normalizedName;
            if (!class_exists($className)) {
                $moduleName = $this->metadata->getScopeModuleName($scope);
                if ($moduleName) {
                    $className = '\\Core\\Modules\\' . $moduleName . '\\Acl\\' . $normalizedName;
                } else {
                    $className = '\\Core\\Acl\\' . $normalizedName;
                }
                if (!class_exists($className)) {
                    $className = '\\Core\\Core\\Acl\\Base';
                }
            }
