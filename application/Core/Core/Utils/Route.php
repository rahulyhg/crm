...
class Route
{
    ...
    protected $paths = array(
        'corePath' => 'application/Core/Resources/routes.json',
        'modulePath' => 'application/Core/Modules/{*}/Resources/routes.json',
        'customPath' => 'custom/Core/Custom/Resources/routes.json',
    );
    ...
    protected function init()
    {
        if (file_exists($this->cacheFile) && $this->getConfig()->get('useCache')) {
            ...
            if ($this->getConfig()->get('useCache')) {
                ...
                if ($result == false) {
                    throw new \Core\Core\Exceptions\Error('Route - Cannot save unified routes');
                }
            }
        }
    }
