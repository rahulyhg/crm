...
class AclManager extends \Core\Core\AclManager
{
    protected $tableClassName = '\\Core\\Core\\AclPortal\\Table';
    ...
    public function getImplementation($scope)
    {
        if (empty($this->implementationHashMap[$scope])) {
            ...
            $className = '\\Core\\Custom\\AclPortal\\' . $normalizedName;
            if (!class_exists($className)) {
                $moduleName = $this->getMetadata()->getScopeModuleName($scope);
                if ($moduleName) {
                    $className = '\\Core\\Modules\\' . $moduleName . '\\AclPortal\\' . $normalizedName;
                } else {
                    $className = '\\Core\\AclPortal\\' . $normalizedName;
                }
                if (!class_exists($className)) {
                    $className = '\\Core\\Core\\AclPortal\\Base';
                }
            }
