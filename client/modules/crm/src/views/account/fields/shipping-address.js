...
Core.define('Crm:Views.Account.Fields.ShippingAddress', 'Views.Fields.Address', function (Dep) {
    ...
    return Dep.extend({
        ...
        copy: function (fieldFrom) {
            var attrList = Object.keys(this.getMetadata().get('fields.address.fields')).forEach(function (attr) {
                destField = this.name + Core.Utils.upperCaseFirst(attr);
                sourceField = fieldFrom + Core.Utils.upperCaseFirst(attr);
