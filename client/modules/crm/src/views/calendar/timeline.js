...
Core.define('crm:views/calendar/timeline', ['view', 'lib!vis'], function (Dep, Vis) {
    ...
    return Dep.extend({
        ...
        data: function () {
            var scopeFilterList = Core.Utils.clone(this.scopeList);
        ...
        setup: function () {
            ...
            this.colors = Core.Utils.clone(this.getMetadata().get('clientDefs.Calendar.colors') || this.colors || {});
            ...
            this.scopeList = this.getConfig().get('calendarEntityList') || Core.Utils.clone(this.scopeList) || [];
            ...
            if (this.header) {
                this.enabledScopeList = this.getStoredEnabledScopeList() || Core.Utils.clone(this.scopeList);
            } else {
                this.enabledScopeList = this.options.enabledScopeList || Core.Utils.clone(this.scopeList);
            }
        ...
        initUserList: function () {
            if (this.options.userList) {
                this.userList = Core.Utils.clone(this.options.userList);
        ...
        storeUserList: function () {
            this.getPreferences().save({
                'sharedCalendarUserList': Core.Utils.clone(this.userList)
            }, {patch: true});
        },
        ...
        getSharedCalenderUserList: function () {
            var list = Core.Utils.clone(this.getPreferences().get('sharedCalendarUserList'));
        ...
        fetchEvents: function (from, to, callback) {
            Core.Ui.notify(this.translate('Loading...'));
