...
Core.define('views/stream/record/edit', 'views/record/base', function (Dep) {
    ...
    return Dep.extend({
        ...
        beforeSave: function () {
            Core.Ui.notify(this.translate('posting', 'messages'));
            this.$post.addClass('disabled');
        },

        afterSave: function () {
            Core.Ui.success(this.translate('Posted'));
            if (this.options.interactiveMode) {
                this.model.clear();
                this.model.set('targetType', 'self');
                this.model.set('type', 'Post');
