    ...
    _.extend(App.prototype, {
            ...
            if (!(name in this.controllers)) {
                try {
                    ...
                    if (!className) {
                        ...
                        className = Core.Utils.composeClassName(module, name, 'controllers');
                    }
                    Core.require(className, function (controllerClass) {
        ...
        initView: function () {
            ...
            this.viewLoader = function (viewName, callback) {
                Core.require(Core.Utils.composeViewClassName(viewName), callback);
            }.bind(this);
            ...
            $.ajaxSetup({
                beforeSend: function (xhr, options) {
                    if (!options.local && self.url) {
                        options.url = Core.Utils.trimSlash(self.url) + '/' + options.url;
                    }
                    ...
                    if (self.auth !== null) {
                        ...
                        xhr.setRequestHeader('Core-Authorization', self.auth);
                    }
                },
                dataType: 'json',
                timeout: 60000,
                ...
                switch (xhr.status) {
                    case 0:
                        if (xhr.statusText == 'timeout') {
                            Core.Ui.error(self.language.translate('Timeout'));
                        }
                        break;
                    case 200:
                        Core.Ui.error(self.language.translate('Bad server response'));
                    ...
                    case 401:
                        if (!options.login) {
                            if (self.auth) {
                                ...
                                Core.Ui.error(self.language.translate('Auth error'));
                            }
                        }
                        break;
                    case 403:
